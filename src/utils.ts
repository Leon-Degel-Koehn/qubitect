import { Stabilizer, KetState, KetZero, Circuit, Gate, KetOne, KetPlus, KetMinus, KNOWN_STATES, UnknownKetState } from "./types.js";
...
export const stateFromStabilizer = (stabilizer: Stabilizer[], bestEffort = true): KetState[] => {
    const numQubits = stabilizer[0].x_part.length;
    for (let comboLen = 1; comboLen < 2 * numQubits; comboLen++) { // just a heuristic on the upper loop bound
        for (let stateCombo of cartesianPower(KNOWN_STATES, comboLen)) {
            const affectedQubits = stateCombo.map((s) => s.stabilizer[0]).reduce((acc, curr) => acc + curr.x_part.length, 0);
            if (affectedQubits > numQubits)
                continue;
            ...
            for (let i = 0, currQubit = 0; i < stateCombo.length; i++) {
                ...
                for (let targetQubit = currQubit; targetQubit - currQubit < substate.stabilizer[0].x_part.length; targetQubit++) {
                    targetQubits.push(targetQubit);
                }
                currQubit += targetQubits.length;
                currStabilizer = currStabilizer.concat(substate.stabilizer.map((generator) => generator.onQubits(targetQubits, numQubits)));
            }
            let contains = true;
            ...
            contains = contains && stabilizer.every((s) => isInStabilizerSet(s, currStabilizer));
            if (contains) {
                return stateCombo;
            }
        }
    ...
    if (bestEffort) {
        ...
        for (let subset of qubitSubsets) {
            const subsetStabilizer = stabilizer.map((s) => new Stabilizer(s.phase,
                s.x_part.filter((_, idx) => subset.includes(idx)),
                s.z_part.filter((_, idx) => subset.includes(idx)))).filter((s) => !s.x_part.every((e) => e == 0) || !s.z_part.every((e) => e == 0));
            const stabilizedState = stateFromStabilizer(subsetStabilizer, false);
            if (!(stabilizedState.every((state) => state == UnknownKetState))) {
                let result: KetState[] = [];
                for (let i = 0; i < numQubits;) {
                    if (subset.includes(i)) {
                        let stabilizedQubits = stabilizedState[0].stabilizer[0].x_part.length;
